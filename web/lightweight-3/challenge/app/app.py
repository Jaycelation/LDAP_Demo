...
def search():
    ...
    cmd = f"ldapsearch -x -H ldap://localhost -b 'ou=prisms,dc=rat,dc=ctf' -D cn=admin,dc=rat,dc=ctf -s sub '(&(objectclass=prismProduct)(cn={prism}))' -w {ADMIN_PASSWORD}"
...
def login():
    if request.method == 'POST':
        ...
        conn.search('ou=people,dc=rat,dc=ctf', f'(&(employeeType=active)(uid={username})(userPassword={password}))', attributes=['uid', 'description'])
